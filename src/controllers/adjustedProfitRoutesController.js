...
const { adjustedProfitRoutesSQL } = require('../queries/adjustedProfitRoutes');
...
async function getAdjustedProfitRoutes(req, res) {
  const year = req.query.year || 2023;
  const limit = req.query.limit || 20;
  const db = (req.query.db || 'both').toLowerCase();

  const sqlPg = adjustedProfitRoutesSQL(year, limit, 'pg');
  const sqlMonet = adjustedProfitRoutesSQL(year, limit, 'monet');
  ...
  try {
    if (db === 'pg') {
      ...
      return res.json({
        ...
        query: { sql: sqlPg, params: { year, limit } },
    ...
    if (db === 'monet') {
      ...
      return res.json({
        db: 'monet',
        metrics: buildMetrics(perf),
        query: { sql: sqlMonet, params: { year, limit } },
        result: perf.result,
        error: perf.error
      });
    }
...
module.exports = { getAdjustedProfitRoutes };