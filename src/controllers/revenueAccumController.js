...
const { revenueAccumulatedSQL } = require('../queries/revenueAccumulated');
...
async function getRevenueAccumulated(req, res) {
  const year = req.query.year || 2023;
  const db = (req.query.db || 'both').toLowerCase();
  const sql = revenueAccumulatedSQL(year);
