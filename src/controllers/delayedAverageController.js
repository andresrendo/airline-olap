...
const { delayedAverageSQL } = require('../queries/delayedAverage');
...
async function getDelayedAverage(req, res) {
  const year = req.query.year || 2023;
  const db = (req.query.db || 'both').toLowerCase();
  const sql = delayedAverageSQL(year);
...
module.exports = { getDelayedAverage };