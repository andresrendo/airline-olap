...
const { leastTravelledNationalitySQL } = require('../queries/leastTravelledNationality');
...
async function getLeastTravelledNationality(req, res) {
  const year = req.query.year || 2023;
  const limit = req.query.limit || 1;
  const db = (req.query.db || 'both').toLowerCase();
  const sql = leastTravelledNationalitySQL(year, limit);
...
module.exports = { getLeastTravelledNationality };