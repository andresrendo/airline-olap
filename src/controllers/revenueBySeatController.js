...
const { revenueBySeatClassAndMonthSQL } = require('../queries/revenueBySeat');
...
async function getRevenueBySeatClassAndMonth(req, res) {
  const year = req.query.year || 2023;
  const db = (req.query.db || 'both').toLowerCase();
  const sql = revenueBySeatClassAndMonthSQL(year);
...
module.exports = { getRevenueBySeatClassAndMonth };