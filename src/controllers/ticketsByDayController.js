...
const { ticketsByWeekdaySQL } = require('../queries/ticketsByWeekday');
...
async function getTicketsByWeekday(req, res) {
  const year = req.query.year || 2023;
  const limit = req.query.limit || 10;
  const db = (req.query.db || 'both').toLowerCase();
  const sql = ticketsByWeekdaySQL(year, limit);
...
module.exports = { getTicketsByWeekday };